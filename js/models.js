...
class StoryList {
  ...
  async deleteStory(user, id){
    try {
      ...
      const newStories = storyList.stories.filter(story => story.storyId !== id);
...
class User {
  ...
  async addRemoveFavorite(id){
    try {
      const isCurrentFavorite = currentUser.favorites.some(fav => id === fav.storyId);
      const method = isCurrentFavorite ? 'DELETE' : 'POST';
      const response = await axios({
      ...
      console.error("addRemoveFavorite failed", err);
      return null;
    }
  }
